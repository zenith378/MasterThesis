...
The cluster counters are configured to be read at an arbitrary frequency, allowing flexibility in data collection. In the current software implementation, these counters are read every three seconds.
...
Each one of the different bunch crossing counters is read and stored, but it is convenient to define already the background-subtracted cluster counters, used for the construction of the local primitives, as: 
\begin{equation}
