...
%This theory correctly classified the Kaons (mesons containing a strange quark), but could not explain their behaviour. In fact, one Kaon could decay in a pair of pions (positive parity) or in three pions (negative parity). Since the initial state is the same, this implied a violation of the parity symmetry. Initially, it was thought that two different particles $\theta$ and $\tau$ existed
%fundamental symmetries such as the charge conjugation $\mathcal{C}$ and parity $\mathcal{P}$. The $\mathcal{CP}$ violation suggests that if a particle is replaced with its antiparticle and if its spatial coordinates are inverted, the laws of physics should change.
%, puzzle emerged as one of the main pro   
...
The Standard Model represents today what early philosophy speculated almost 3000 years ago: the matter is composed by unbreakable elements and it is our quest as particle physicists to study the interactions that govern their behaviour.
...
There are three ways we can observe $\mathcal{CP}$ violation in particle physics experiment:
 ...
 The $\mathcal{CP}$ violation is incorporated into the Standard Model through a complex phase in the CKM matrix for quarks and the Pontecorvo-Maki-Nakagawa-Sakata (PMNS) matrix for neutrinos mixing, a process analogous to quark mixing. The CKM matrix can be represented in the Wolfenstein parametrisation as 
 \begin{equation}
 V_{CKM} = 
     \begin{pmatrix}
         V_{ud} & V_{us} & V_{ub}\\
...
%In Run~3, these processes are now handled by GPUs, that multiplex  information onto different threads, enabling further parallelisation.
...
%, which are known to depend on the luminous region position. 
